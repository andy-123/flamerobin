//	Subject : IBPP, internal EventsImplFb3 class implementation
//
//	COMMENTS
//
...
const size_t EventsImplFb3::MAXEVENTNAMELEN = 127;

//	(((((((( OBJECT INTERFACE IMPLEMENTATION ))))))))

void EventsImplFb3::Add(const std::string& eventname, IBPP::EventInterface* objref)
{
	if (eventname.size() == 0)
...
void EventsImplFb3::Drop(const std::string& eventname)
{
	if (eventname.size() == 0)
		throw LogicExceptionImpl("EventsImplFb3::Drop", _("Zero length event names not permitted"));
	if (eventname.size() > MAXEVENTNAMELEN)
		throw LogicExceptionImpl("EventsImplFb3::Drop", _("Event name is too long"));
	...
	typedef EventBufferIteratorFb3<Buffer::iterator> EventIterator;
...
void EventsImplFb3::List(std::vector<std::string>& events)
{
	events.clear();
...
IBPP::Database EventsImplFb3::DatabasePtr() const
{
...
void EventsImplFb3::Queue()
{
	if (! mQueued)
	{
		if (mDatabase->GetHandle() == 0)
			throw LogicExceptionImpl("EventsImplFb3::Queue",
		...
		if (vector.Errors())
		{
			...
			mQueued = false;
			throw SQLExceptionImpl(vector, "EventsImplFb3::Queue",
				_("isc_que_events failed"));
		}
	}
}

void EventsImplFb3::Cancel()
{
	if (mQueued)
	{
		if (mDatabase->GetHandle() == 0) throw LogicExceptionImpl("EventsImplFb3::Cancel",
			_("Database is not connected"));
	    ...
	    if (vector.Errors())
		{
	    	...
	    	throw SQLExceptionImpl(vector, "EventsImplFb3::Cancel",
...
void EventsImplFb3::FireActions()
{
	if (mTrapped)
	{
		typedef EventBufferIteratorFb3<Buffer::iterator> EventIterator;
				 ...
				 ++oit, ++eit, ++rit)
		{
			if (eit == EventIterator(mEventBuffer.end())
				...
				throw LogicExceptionImpl("EventsImplFb3::FireActions", _("Internal buffer size error"));
...
void EventsImplFb3::EventHandler(const char* object, short size, const char* tmpbuffer)
{
	...
	EventsImplFb3* evi = (EventsImplFb3*)object;	// Ugly, but wanted, c-style cast
...
void EventsImplFb3::AttachDatabaseImpl(DatabaseImplFb3* database)
{
	if (database == 0) throw LogicExceptionImpl("EventsImplFb3::AttachDatabase",
...
void EventsImplFb3::DetachDatabaseImpl()
{
...
EventsImplFb3::EventsImplFb3(DatabaseImplFb3* database)
{
	mDatabase = 0;
	mId = 0;
