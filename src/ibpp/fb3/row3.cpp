...
/*
void RowImplFb3::Set(int param, const IBPP::Value& value)
{
	if (mDescrArea == 0)
...
{
	if (varnum < 1 || varnum > mDescrArea->sqld)
		throw LogicExceptionImpl("RowImplFb3::SetValue", _("Variable index out of range."));
	if (value == 0)
	...
	switch (var->sqltype & ~1)
	{
		case SQL_TEXT :
			...
			else throw WrongTypeImpl("RowImplFb3::SetValue", var->sqltype, ivType,
		...
		case SQL_FLOAT :
			if (ivType != ivFloat || var->sqlscale != 0)
				throw WrongTypeImpl("RowImplFb3::SetValue", var->sqltype, ivType,
									_("Incompatible types."));
...
{
	if (varnum < 1 || varnum > mDescrArea->sqld)
		throw LogicExceptionImpl("RowImplFb3::GetValue", _("Variable index out of range."));
	...
	switch (var->sqltype & ~1)
	{
			...
			else throw WrongTypeImpl("RowImplFb3::GetValue", var->sqltype, ivType,
										_("Incompatible types."));
			break;
		...
		case SQL_FLOAT :
			if (ivType != ivFloat)
				throw WrongTypeImpl("RowImplFb3::GetValue", var->sqltype, ivType,
										_("Incompatible types."));
			value = var->sqldata;
			break;
		...
		for (int i = 0; i < mDescrArea->sqln; i++)
		{
			...
			if (var->sqldata != 0)
			{
				switch (var->sqltype & ~1)
				{
					...
					default : throw LogicExceptionImpl("RowImplFb3::Free",
								_("Found an unknown sqltype !"));
				}
			}
...
void RowImplFb3::AllocVariables()
{
	int i;
	for (i = 0; i < mDescrArea->sqld; i++)
	{
		...
		switch (var->sqltype & ~1)
		{
			...
			case SQL_DEC34 :	var->sqldata = (char*) new FB_DEC34_t;
								...
								break;;
			default : throw LogicExceptionImpl("RowImplFb3::AllocVariables",
						_("Found an unknown sqltype !"));
		}
...
RowImplFb3& RowImplFb3::operator=(const RowImplFb3& copied)
{
	...
	for (int i = 0; i < mDescrArea->sqld; i++)
	{
		...
		switch (var->sqltype & ~1)
		{
			...
			default : throw LogicExceptionImpl("RowImplFb3::Ctor",
						_("Found an unknown sqltype !"));
		}
...
RowImplFb3::RowImplFb3(int dialect, int n, DatabaseImplFb3* db, TransactionImplFb3* tr)
	: mDescrArea(0)
{
	Resize(n);
	mDialect = dialect;
